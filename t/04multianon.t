...
##FindBin->again;
##my $path = $Bin.'/'.$Script;
##my $unique_id = IPC::SysV::ftok($path);
##print STDERR 'script is '.$path.' and unique id is '.$unique_id;
...
#
sub create_mmap {
	return IPC::Mmap->new('test2_mmap.tmp', 10000, PROT_READ|PROT_WRITE, MAP_SHARED|MAP_ANON);
}

my $mmap = create_mmap;
report_result(defined($mmap), 'create anonymous area');
